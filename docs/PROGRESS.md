...
|---|---|---|---|---|
| **Phase 0** Foundation | Weeks 1–2 | 0 | 22 | In Progress |
...
|---|---|---|---|---|---|---|
| P0-01 | Initialize git repo + GitHub org | Solo | Wk 1 | ☐ |  |  |
| P0-02 | Pick + commit LICENSE (AGPLv3) | Solo | Wk 1 | ☐ |  | Replace stub with full text from gnu.org |
| P0-03 | Bun monorepo with /backend, /frontend, /sdk, /helm, /docs | BE | Wk 1 | ☐ |  |  |
| P0-04 | Pin Bun version in `.tool-versions` and Dockerfile | BE | Wk 1 | ☐ |  |  |
| P0-05 | TypeScript strict + ESLint + Prettier + Husky | FE | Wk 1 | ☐ |  |  |
| P0-06 | Bootstrap Elysia backend with `/health` route | BE | Wk 1 | ☐ |  |  |
| P0-07 | Bootstrap React + Vite frontend | FE | Wk 1 | ☐ |  |  |
| P0-08 | Wire Elysia + React via Eden Treaty | BE+FE | Wk 1 | ☐ |  |  |
| P0-09 | Drizzle + Postgres; first migration with `users` + `tenant_id` | BE | Wk 1 | ☐ |  |  |
| P0-10 | Local docker-compose: Postgres + Valkey | BE | Wk 1 | ☐ |  |  |
| P0-11 | `make dev` script | BE | Wk 1 | ☐ |  |  |
| P0-12 | GitHub Actions CI (lint, typecheck, test, Docker build) | BE | Wk 2 | ☐ |  |  |
| P0-13 | Distroless Bun Dockerfile (multi-arch) | BE | Wk 2 | ☐ |  |  |
| P0-14 | First Helm chart skeleton in `/helm` | BE | Wk 2 | ☐ |  |  |
| P0-15 | Air-gapped install dry-run script in CI | BE | Wk 2 | ☐ |  |  |
| P0-16 | OpenTelemetry instrumentation skeleton | BE | Wk 2 | ☐ |  |  |
| P0-17 | ADR template + first 3 ADRs (stack, license, multi-tenancy) | Solo | Wk 2 | ☐ |  |  |
| P0-18 | Public docs site stub (Mintlify or Nextra) | FE | Wk 2 | ☐ |  |  |
| P0-19 | README, CONTRIBUTING, SECURITY, CODE_OF_CONDUCT | Solo | Wk 2 | ☐ |  |  |
| P0-20 | First Playwright E2E test | FE | Wk 2 | ☐ |  |  |
| P0-21 | Bun + DuckDB load-test spike (100 concurrent queries) | BE | Wk 2 | ☐ |  |  |
| P0-22 | Phase 0 EXIT review | Solo | Wk 2 | ☐ |  |  |

### Phase 0 Exit Criteria

- [ ] All 22 Phase-0 tasks marked ✓
...
- [ ] Air-gapped install dry-run passes — disconnected container starts without errors
- [ ] Bun + DuckDB load-test result documented (continue Bun OR switch query tier to Go)
- [ ] First 3 ADRs committed in `/docs/adr/`
